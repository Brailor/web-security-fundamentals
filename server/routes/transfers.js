...
router.get('/', csurfMW, function(req, res /*, next*/) {
  bounceOutIfLoggedOut(req, res, () => {
    ...
    Account.findAll({
      ...
      let userAccounts = collection.groupBy(allAccounts.map(a => a.get({ plain: true })), acc => acc.user.username);
      //////////////////////////////////////////////////////////////
      //// ↓ EXERCISE 5 SOLUTION GOES HERE
...
router.post('/perform', csurfMW, function(req, res) {
  bounceOutIfLoggedOut(req, res, () => {
    ...
    Promise.all([accountFrom, accountTo].map(accountId => Account.findById(accountId)))
      .then(([aFrom, aTo]) => {
        if (!aFrom || !aTo) {
